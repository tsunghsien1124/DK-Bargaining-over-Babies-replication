/******************************************************************************
 * Table A.7: What covaries with male participation in child care?
 ******************************************************************************/
...
* regress child care participation on different explanatory factors
display "WITH 1 KID / MARRIED"
quietly: regress as_aver married   age_f-firstmale countryFE* if(numkids == 1) [aweight = aweight2], robust noconstant
eststo C1

display "WITH 1 KID / PARTNERED"
quietly: regress as_aver part_pres age_f-firstmale countryFE* if(numkids == 1) [aweight = aweight2], robust noconstant
eststo C2

display "WITH 2+ KIDS / MARRIED"
quietly: regress as_aver married   age_f-firstmale countryFE* if(numkids == 2) [aweight = aweight2], robust noconstant
eststo C3

display "WITH 2+ KIDS / PARTNERED"
quietly: regress as_aver part_pres age_f-firstmale countryFE* if(numkids == 2) [aweight = aweight2], robust noconstant
eststo C4

esttab C1 C2 C3 C4 using "$texres/table_A7.tex", replace ///
    se(%7.4f) b(%7.4f) r2 booktabs compress label nonotes nonumbers nomtitles extracols(3) varwidth(20) ///
    order(married part_pres edu_f edu_m empl_f empl_m child_ins child_fam firstmale female) ///
	nostar ///
	mgroups("with 1 child" "with 2+ children", pattern(1 0 1 0) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) ///
	mlabels("(1)" "(2)" "(1)" "(2)", prefix("{") suffix("}")) ///
	stats(N r2, layout({@} {@}) fmt(a3 %5.3f) labels("Number of Cases" "R-Square")) ///
	addnotes("Standard errors reported in parentheses.") ///
	drop(countryFE* age_f age_f2 age_diff)
