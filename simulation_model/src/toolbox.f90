...
! MODULE matrixtools
!
! Large parts of the procedures were taken and adapted from:
...
!##############################################################################
    ...
    ! 
    ! Adapted from Algorithm Applied Statistics (1973) vol22 no.3
    ...
    !
    ! Adapted from Fortran Code by John Burkardt available as Algorithm
    !    ASA241 from https://people.sc.fsu.edu/~jburkardt/f_src/asa241/asa241.html
    ! Reference:
    !    Michael Wichura,
    ...
    function betaPDF(x, p, q)
        ...
        if(x < 0d0 .or. x > 1d0)then
            ...
            betanorm = exp(log_gamma(p_c) + log_gamma(q_c) - log_gamma(p_c+q_c))
    ...
    !
    ! Adapted from Fortran Code by John Burkardt available as Algorithm
    !    ASA063 from https://people.sc.fsu.edu/~jburkardt/f_src/asa063/asa063.html
    ! Reference:
    ...
    !
    ! Adapted from Fortran Code by John Burkardt available as Algorithm
    !    ASA239 from https://people.sc.fsu.edu/~jburkardt/f_src/asa239/asa239.html
    ! Reference:
    !    B Shea,
    !    Algorithm AS 239:
    ...
    !
    ! Adapted from algorithm AS245 by Allan J. Macleod
    ! Reference:
    !    Allan J. Macleod,
    ...
    function log_gamma(x_in)
        ...
        ! check validity of inputs solution
        if(x < 0d0)then
            call error('log_gamma','x is smaller than zero')
        endif
        ...
        if(x < 1.5d0)then
            ...
            log_gamma = log_gamma + x * ((((r1(5)*y + r1(4))*y + r1(3))*y &
        ...
        elseif(x < 4.0d0)then
            y = (x - 1d0) - 1d0
            log_gamma = y * ((((r2(5)*x + r2(4))*x + r2(3))*x + r2(2))*x &
                + r2(1)) / ((((x + r2(9))*x + r2(8))*x + r2(7))*x + R2(6))

        ! get solution for 4.0 <= x < 12
        elseif(x < 12d0)then
            log_gamma = ((((r3(5)*x + r3(4))*x + r3(3))*x + r3(2))*x + r3(1)) / &
                ((((x + r3(9))*x + r3(8))*x + r3(7))*x + r3(6))
        else
            y = log(x)
            log_gamma = x * (y -1d0) - 0.5d0*y + alr2pi
            if(x <= XLGE)then
                x1 = 1d0/x
                x2 = x1 * x1
                log_gamma = log_gamma + x1 * ((r4(3)*x2 + r4(2))*x2 + r4(1)) / &
                    ((x2 + r4(5))*x2 + r4(4))
            endif
        endif
